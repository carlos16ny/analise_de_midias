...
import json, os, time, gzip, getopt, traceback, sys
        ...
        for subcat in categorie_dict:
            if subcat == 'UNSPECIFIED':
                if categorie_dict[subcat]+total_audience_amount ==0:
                    ...
                    percent_category_dict[subcat]=float(categorie_dict[subcat])/(categorie_dict[subcat]+total_audience_amount)
            elif subcat == 'expats':
                if total_amount == 0:
                    percent_category_dict[subcat]=0                         
                else:
...
#         checking the gender category
    ...
    #                 print interest_filename                              
                ...
                new_low_value = float(raw_values_dict['all']['all'] - raw_values_dict['gender'][high_percent_group])/raw_values_dict['all']['all'] 
...
def getInterestsDemographics(interests_path, list_of_interests):
    ...
    for interest in list_of_interests:
        ...
        if os.path.isfile(interest_filename):
            try:
                input_file = gzip.open(interest_filename)
...
#         break
        ...
        if raw_values_dict == None or percent_values_dict == None:
...
#             calc_error_count+=1         
        ...
        output_file_json.write(json.dumps(percent_values_dict))
...
def main(argv):
    interests_path = '/home/ICEA/<cpf>/facebook_ads_test/US/'
    verifyDir(interests_path)
...
#     dia e hora da coleta all_<data_hora_coleta>
    list_of_interests = ['6003058819532', '6003096798132']
    getInterestsDemographics(interests_path, list_of_interests)
    
    
#     opening json files
#     json_interest_file = open('%s6003058819532.json' % (interests_path))
#     percent_values_dict = json.loads(json_interest_file.readline())
#     print 'male percentage: %.2f' % percent_values_dict['gender']['male']
#     print 'female percentage: %.2f' % percent_values_dict['gender']['female']
