...
#replace the token and act_id with your personal values
token = ""
act_id = ""
secret = "-"
...
def crawlInterest(interest_id, destination_file_path, account, log_file, exception_log, count_total_requests, count_sequenced_exceptions, count_exceptions, sleep_time):
...
# ********************************** GLOBAL INTEREST ********************************** 
    ...
    count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('all', 'all', interest_id,
         targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
...
#********************************** EDUCATION_STATUSES **********************************  
    ...
    for scholarity_field in search_elements.scholarities:
        ...
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('education_status', scholarity_field, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file)             
...
# ********************************** POLITICAL_ALIGNMENT **********************************                     
    for political_leaning in search_elements.political_alignment:
        ...
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('political_alignment', political_leaning, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file)             
...
# ********************************** RELATIONSHIP_STATUSES **********************************                 
    for relationship_status in search_elements.relationship_statuses:
        ...
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('relationship_status', relationship_status, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)  
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file) 
...
# ********************************** AGE GROUP **********************************
    ...
    for age_group in search_elements.age_intervals:
        if DEBUG_CATEGORIES: print age_group
        if targeting_spec.has_key('age_min'):
            del targeting_spec['age_min']
        if targeting_spec.has_key('age_max'):                                
        ...
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('age_intervals', age_group, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)  
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file)             
...
# ********************************** RACE **********************************   
    for race in search_elements.racial_affinities:
        if DEBUG_CATEGORIES: print race
        exclusions_race = []  
        behaviors_race = []
        flexible_spec = []                              
        if race == 'other':
            ...
            count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('racial_affinities', race, interest_id,
                 targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
                 count_exceptions, sleep_time)           
            if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
                removingIncompleteOutputFile(output_file, destfilename, log_file)                 
...
#         behaviors_expats.append(search_elements.expats[expats]) 
        ...
        targeting_spec['flexible_spec']=flexible_spec 
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('expat', expats, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file)             
...
# ********************************** GENDER **********************************                                                                                           
    for gender in search_elements.genders:
        ...
        count_total_requests, count_sequenced_exceptions, count_exceptions, audience_below_1000 = executeRequest('gender', gender, interest_id,
             targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, 
             count_exceptions, sleep_time)
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
            removingIncompleteOutputFile(output_file, destfilename, log_file)             
...
def executeRequest(category, value, interest_id, targeting_spec, account, exception_log, writer, output_file, count_total_requests, count_sequenced_exceptions, count_exceptions, sleep_time):
...
def crawlDemographics(output_file_path, sleep_time):  
    ...
    exception_log_file = open('%sexception_log_all.txt' % (destination_file_path), 'w')  
  

    if secret != '-':
        FacebookAdsApi.init(access_token=token, app_secret=secret, api_version='v3.1')
    else:
...
#         crawlInterest(interest_id, destination_file_path, account, log_file, exception_log_file, sleep_time)   
        count_total_requests, count_sequenced_exceptions, count_exceptions = crawlInterest(interest_id, destination_file_path, account, log_file, exception_log_file, count_total_requests, count_sequenced_exceptions, count_exceptions, sleep_time)               
        if count_sequenced_exceptions == MAXIMUM_EXCEPTIONS:
...
def main(argv):
    #output_file_path = '/home/ICEA/<cpf>/facebook_ads_test/' 
    output_file_path = '/home/filipe/Desktop/facebook_ads_test/' 
    sleep_time = 1
